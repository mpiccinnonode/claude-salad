...
2. If found:
   ...
   b. **Validate:**
      - Check that all `source` files in `locked` entries exist. If any are
        missing, warn the user and offer: continue without that locked file, or
        abort.
      - Check for targets appearing in both `locked` and `inject`. If found,
        warn: "Target `{path}` is locked --- inject rules for it will be
        ignored."
...
3. If YAML is malformed, warn the user and offer to continue without the
   template or abort.
...
All other Phase 3 behavior is unchanged:

- `--dry-run` shows the report without writing.
- Apply writes both locked and generated files.
- User approval is required before writing.
...
A locked entry references a source file that does not exist. The skill warns
the user per-file and offers to skip that entry or abort entirely.

### Malformed YAML

The skill warns and offers to continue without the template or abort. It does
not attempt partial parsing.
